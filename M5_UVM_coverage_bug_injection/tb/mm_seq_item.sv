    ...
    function void compute_expected();
        for (int r = 0; r < N; r++)
            for (int c = 0; c < N; c++) begin
                ...
                for (int k = 0; k < N; k++)
`ifdef BUG4
                    // BUG 4 (ACTIVE): Reference model uses addition instead of
                    // multiplication.  C[r][c] += A[r][k] + B[k][c] instead of
                    // A[r][k] * B[k][c].  The monitor calls compute_expected() on
                    // its own seq_item, so it also produces the wrong expected C.
                    // The DUT correctly computes A×B, causing uvm_error for every
                    // non-zero element in every transaction.
