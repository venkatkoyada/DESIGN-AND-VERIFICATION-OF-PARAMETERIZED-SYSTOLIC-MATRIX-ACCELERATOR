...
//   - build_phase  : instantiate all sub-components via UVM factory (type_id::create)
//   - connect_phase: wire driver ↔ sequencer TLM ports
//   - run_phase    : nothing at agent level (professor: "Nothing in the run_phase at Agent level")
//
// Sub-components created inside agent:
//   mm_sequencer seqr  — arbitrates sequence items to driver
//   mm_driver    drv   — drives DUT signals via virtual interface
//   mm_monitor   mon   — passively observes DUT, broadcasts via analysis port
//
// TLM connections made here:
//   drv.seq_item_port  ──→  seqr.seq_item_export
//   (monitor_port → scoreboard connection is made in mm_env connect_phase)
