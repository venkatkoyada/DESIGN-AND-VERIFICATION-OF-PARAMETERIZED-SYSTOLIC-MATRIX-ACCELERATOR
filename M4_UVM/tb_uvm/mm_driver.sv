...
//   - extends uvm_driver#(mm_seq_item)
//   - `uvm_component_utils (component type, two constructor args)
//   - build_phase: gets virtual interface from uvm_config_db
//   - run_phase: apply_reset() once, then forever:
//       get_next_item(req) → drive_transaction(req) → item_done()
//
// DUT Protocol (per transaction):
//   Step 1 : Assert start=1 for 1 cycle
//   Step 2 : Wait for ready=1  (controller enters LOAD state)
//   Step 3 : Drive N cycles — a_in=col_k_of_A, b_in=row_k_of_B, valid=1
//   Step 4 : De-assert valid_in_upstream
//   Step 5 : Wait for result_valid=1 (controller in DONE state)
//            → item_done() called AFTER result_valid so monitor can capture result
//
// Timing convention: @(posedge clk); #1; before every signal assignment
// to place assignments after the active clock edge (avoids delta-cycle races).
